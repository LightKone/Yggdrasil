...
#include "block_data_transfer_plumtree.h"
...
static void request_dissemination(file_meta* meta, block_data_transfer_state* state) {
    ...
    for(int i = 0; i < blocks; i++) {
        ...
        int filename_size = strlen(meta->filename);
        dissemmination_request_add_to_header(d_req, &filename_size, sizeof(int));
        dissemmination_request_add_to_header(d_req, meta->filename, filename_size);
        dissemmination_request_add_to_header(d_req, &meta->mode, sizeof(mode_t));
        dissemmination_request_add_to_header(d_req, &meta->file_size, sizeof(off_t));
        dissemmination_request_add_to_header(d_req, &i, sizeof(int)); //block number
        dissemmination_request_add_to_header(d_req, &blocks, sizeof(int));
        ...
        dissemmination_request_add_to_header(d_req, &block_size, sizeof(int));
...
static void provide_msg_body(YggRequest* req, block_data_transfer_state* state) {
    plumtree_request_msg_body* p_req = req->payload;

    void* ptr = p_req->req->header + sizeof(uint16_t);

    file_meta meta;
    int filename_size;
    memcpy(&filename_size, ptr, sizeof(int)), ptr+= sizeof(int);
    meta.filename= malloc(filename_size);
    memcpy(meta.filename, ptr, filename_size), ptr+= filename_size+ sizeof(mode_t); //skip mode
    memcpy(&meta.file_size, ptr, sizeof(off_t)), ptr+= sizeof(off_t);
    memcpy(&meta.block, ptr, sizeof(int)), ptr+= sizeof(int);
    memcpy(&meta.n_blocks, ptr, sizeof(int)), ptr += sizeof(int);
    int block_size;
    memcpy(&block_size, ptr, sizeof(int));

#if defined TEST_LOCAL_TRANSFER
    change_file_location(&meta, state);
#endif

    meta.fd = open_file(meta.filename);

    printf("===================> Answering body request for block %d of file %s\n", meta.block, meta.filename);
...
static void process_request(YggRequest* req, block_data_transfer_state* state) {
    ...
    } else if(req->proto_origin == state->dissemination_proto && req->request_type == PLUMTREE_REQUEST_MSG_BODY)
...
proto_def* block_data_transfer_plumtree_init(block_data_transfer_args* args) {
