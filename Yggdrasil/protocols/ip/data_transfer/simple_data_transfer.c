...
#include "simple_data_transfer.h"

typedef struct __simple_data_transfer_state {
...
}simple_data_transfer_state;

typedef enum _simple_data_transfer_msgs {
    BROADCAST_FILE,
    DIRECT_FILE,
    HANDSHAKE,
    HANDSHAKE_REPLY
}simple_data_transfer_msgs;

static uint16_t codify(simple_data_transfer_msgs msg_type) {
    return htons(msg_type);
}

static simple_data_transfer_msgs decodify(uint16_t code) {
    return ntohs(code);
}
...
static bool read_file_to_mem(void* buffer, file_meta* meta) {


    void* ptr;
    if((ptr = mmap(NULL, meta->file_size, PROT_READ, MAP_PRIVATE, meta->fd, 0))) {
        memcpy(buffer, ptr, meta->file_size);
        munmap(ptr, meta->file_size);
...
static bool write_file_to_disk(void* buffer, file_meta* meta) {
    ...
    off_t writen = 0;
    int n;
    while(writen < meta->file_size) {
        if((n = write(meta->fd, buffer + writen, meta->file_size - writen)) < 0) {
...
static void request_dissemination(file_meta* meta, simple_data_transfer_state* state) {
    ...
    req.payload = realloc(req.payload, req.length+meta->file_size);
    if(read_file_to_mem(req.payload+req.length, meta))
        req.length += meta->file_size;
    else{
...
static void process_request(YggRequest* req, simple_data_transfer_state* state) {
    if(req->request_type == DISSEMINATE_FILE) {
        ...
        if(meta && meta->file_size < USHRT_MAX - 100) {
...
static void change_file_location(file_meta* meta, simple_data_transfer_state* state) {
...
static void process_file(YggMessage* msg, void* ptr, simple_data_transfer_state* state) {
...
static void process_msg(YggMessage* msg, simple_data_transfer_state* state) {
    uint16_t code;
    void* ptr = YggMessage_readPayload(msg, NULL, &code, sizeof(uint16_t));
    simple_data_transfer_msgs msg_type = decodify(code);
...
proto_def* simple_data_transfer_init(simple_data_transfer_args* args) {
    ...
    simple_data_transfer_state* state = malloc(sizeof(simple_data_transfer_state));
    state->my_id = PROTO_SIMPLE_DATA_TRANSFER;
...
simple_data_transfer_args* simple_data_transfer_args_init(short dissemination_proto, short dissemination_request, char* dir){
    simple_data_transfer_args* args = malloc(sizeof(simple_data_transfer_args));
    args->dir = dir;
