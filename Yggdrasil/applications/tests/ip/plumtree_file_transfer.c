...
static void create_file(YggMessage* msg, void* ptr) {
    int file_name_size;

    ptr = YggMessage_readPayload(msg, ptr, &file_name_size, sizeof(int));

    char filename[file_name_size];
    ptr = YggMessage_readPayload(msg, ptr, filename, file_name_size);

    int file_len;
    ptr = YggMessage_readPayload(msg, ptr, &file_len, sizeof(int));

    char* buffer = malloc(file_len);
    YggMessage_readPayload(msg, ptr, buffer, file_len);


    //TODO create and open files from different locations
    ...
    char* f = concat("/", file);
    char* dir_filename = concat(dir, f);
    free(f);

    int fd = open(dir_filename, O_CREAT | O_RDWR);
    ftruncate(fd, file_len+1);
    
    void* f_addr;

    if ((f_addr = mmap(NULL, file_len+1, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0)) == (caddr_t) -1)
    {
        perror ("mmap error for output ");

    }

    memcpy(f_addr, buffer, file_len);
    msync(f_addr, file_len+1, MS_SYNC);
        ...
        if(elem.type == YGG_MESSAGE) {
            ...
            switch (command) {
                ...
                case SEND:
                    ...
                    break;
                case FILE_TRANSFER:
                    ygg_log("APP", "FILE", "received file");
                    create_file(&elem.data.msg, ptr);
                    break;
            }
