...
int main(int argc, char* argv[]) {
	...
	short proto_agg_id = PROTO_AGG_LIMOSENSE;
	limosense_args* args = limosense_args_init(PROTO_FAULT_DETECTOR_DISCOVERY_ID, false, TEST_INPUT, 2, 0);
	registerProtocol(proto_agg_id, limosense_init, args);
	limosense_args_destroy(args);
